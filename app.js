/**
 * Stock Tracker — Main Application
 * ================================
 * TABLE OF CONTENTS (search "// ---" to jump between sections)
 *
 *   Line ~  1  DOM refs & state variables
 *   Line ~ 22  User-scoped storage helpers (userGet/userSet)
 *   Line ~ 50  Error toast notification system
 *   Line ~ 86  API Keys (save/load)
 *   Line ~148  Guide modal
 *   Line ~165  Search (typeahead, suggestions)
 *   Line ~212  Add / Remove / Select stock
 *   Line ~247  Sidebar rendering
 *   Line ~270  Detail panel (renderDetail dispatcher)
 *   Line ~349  Tile drag-and-drop reordering
 *   Line ~622  Download PDF
 *   Line ~726  Tile Visibility Manager
 *   Line ~834  Detail: Header
 *   Line ~844  Detail: About Company
 *   Line ~879  Detail: AI Verdict
 *   Line ~1241 Detail: KPIs
 *   Line ~1256 Detail: TradingView Widget
 *   Line ~1287 Detail: PE Chart
 *   Line ~1319 Detail: Technical Indicators
 *   Line ~1430 Detail: Earnings
 *   Line ~1570 Detail: Cash Flow
 *   Line ~1681 Detail: Balance Sheet
 *   Line ~1785 Detail: Dividends
 *   Line ~1833 Detail: ETF Holdings
 *   Line ~1868 Detail: Revenue & Income Trend
 *   Line ~2019 Detail: Insider Trading
 *   Line ~2072 Detail: Company Fundamentals & Sentiment
 *   Line ~2326 Detail: AI Analysis
 *   Line ~2375 Detail: Analyst Ratings
 *   Line ~2550 Detail: Macro & Economy
 *   Line ~2647 Detail: Earnings Call Summary
 *   Line ~2827 Detail: Peer Comparison
 *   Line ~3033 Detail: Sector Relative Strength
 *   Line ~3081 Detail: Valuation Scorecard
 *   Line ~3139 Detail: EPS Estimate Revisions
 *   Line ~3231 Detail: Options & Volatility
 *   Line ~3286 Detail: Watchlist Correlation Matrix
 *   Line ~3342 Detail: Fair Value Range
 *   Line ~3420 Detail: Price Alerts
 *   Line ~3542 Detail: News
 *   Line ~3588 Data loading (loadQuote, loadAll, etc.)
 *   Line ~3888 Auto-refresh
 *   Line ~3998 Market Ticker Bar
 *   Line ~4049 Utility (fmtNum, etc.)
 *   Line ~4058 Login screen
 *   Line ~4159 Portfolio Brief renderer
 *   Line ~4455 Morning Report (batch Senior Analyst + email)
 *   Line ~4814 Init
 *   Line ~4873 Bridge for advisor.js
 */
(function() {
  ...
  // ===== Bridge for advisor.js (loaded as separate script) =====
  // Exposes minimal interface so advisor.js can access app state
  window._appBridge = {
    getTrackedStocks: function() { return trackedStocks; },
